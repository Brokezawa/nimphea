...
proc toQ7*(dst: var openArray[q7_t], src: openArray[float32]) {.inline.} =
  let n = min(dst.len, src.len)
  if n > 0: arm_float_to_q7(cast[ptr float32_t](addr src[0]), cast[ptr q7_t](addr dst[0]), n.uint32)
...
proc toFloat*(dst: var openArray[float32], src: openArray[q7_t]) {.inline.} =
  let n = min(dst.len, src.len)
  if n > 0: arm_q7_to_float(cast[ptr q7_t](addr src[0]), cast[ptr float32_t](addr dst[0]), n.uint32)
